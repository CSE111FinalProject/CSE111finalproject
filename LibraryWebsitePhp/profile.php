			...
			<hr style='border-top:1px dotted #ccc;'/>
					...
					<div class='form-group' style='overflow:auto; height: 450px;'>
						...
						<?php
							...
							$statement = $db->prepare('SELECT "b_title", "l_loandate","l_fees","l_loanlength","l_loanid" FROM "cardholder", "loans","books","Loanbooks" WHERE "c_cardid" = "l_cardid" AND "c_username" = ? AND "l_loanid" = "lb_loanid" AND "lb_bookid" = "b_bookid"');
							$statement->bindValue(1,$login_session);
							$statement1 = $db->prepare('SELECT "m_title","l_loandate","l_fees","l_loanlength","l_loanid"  FROM "cardholder", "loans","movies","Loanmovies" WHERE "c_cardid" = "l_cardid" AND "c_username" = ? AND "l_loanid" = "lm_loanid" AND "lm_movieid" = "m_movieid"');
							...
							$db->exec('COMMIT');
							list($material,$loandate,$fees,$time, $loanid) = $result->fetchArray(PDO::FETCH_NUM);
							list($material1,$loandate1,$fees1,$time1,$loanid1) = $result1->fetchArray(PDO::FETCH_NUM);
						...
						//add more columns if needed or change column need
								...
								echo'</thead>';
								if($material){
									echo'<tr><td>'.$material.'</td><td>'.$loandate.'</td><td>'.'$'.$fees.'</td><td>'.$time.'</td><td>'
									...
									while($fetch=$result->fetchArray()){
										echo'<tr><td>'.$fetch['b_title'].'</td><td>'.$fetch['l_loandate'].'</td><td>'.'$'.$fetch['l_fees'].'</td><td>'.$fetch['l_loanlength'].'</td><td>'
										.'<form action = "return_form.php" method="post">
										<input type="hidden" name="id" value= "'.$fetch['l_loanid'].'">
										<input type="hidden" name="type" value= "Book">
										<input type="submit" name="return" value="return">
									 </form>'.
										'</td></tr>';
						...
						//add more columns if needed or change column need
								...
								if($material1){
									...
									echo'<tr><td>'.$material1.'</td><td>'.$loandate1.'</td><td>'.'$'.$fees1.'</td><td>'.$time1.'</td><td>'
									.'<form action = "return_form.php" method="post">
									<input type="hidden" name="id" value= "'.$loanid1.'">
									<input type="hidden" name="type" value= "Movie">
									<input type="submit" name="return" value="return">
								 </form>'.
									'</td></tr>';
									while($fetch=$result1->fetchArray()){
										echo '<tr><td>'.$fetch['m_title'].'</td><td>'.$fetch['l_loandate'].'</td><td>'.'$'.$fetch['l_fees'].'</td><td>'.$fetch['l_loanlength'].'</td><td>'
										.'<form action = "return_form.php" method="post">
										<input type="hidden" name="id" value= "'.$fetch['l_loanid'].'">
										<input type="hidden" name="type" value= "Movie">
										<input type="submit" name="return" value="return">
